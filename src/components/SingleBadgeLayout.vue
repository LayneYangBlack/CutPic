<template>
    <div
        class="p-4 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-8 printable-hidden"
    >
        ...
        <div class="space-y-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <select
                            ...
                            class="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm text-gray-900"
                        >
                            <option
                                v-for="(outer, inner) in sizeMap"
                                :key="inner"
                                :value="inner"
                            >
                                {{ inner }}mm (外圈 {{ outer }}mm)
                            </option>
                        </select>
                    </div>
                    ...
                    <div
                        ...
                        class="p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-700 animate-fade-in"
                    >
                        <p>
                            自动模式将最大化利用纸张空间，并自动计算间隙，实现页面居中对齐。
                        </p>
                    </div>
            ...
            >
                ...
                <div
                    ...
                    :style="{
                        backgroundColor: cropperBgColor,
                        height: '400px',
                    }"
                >
                    <CustomCropper
        ...
        <div class="p-4 border rounded-lg bg-white shadow-sm">
            ...
            <div
                class="bg-gray-200 p-2 a4-preview-container overflow-auto shadow-inner rounded"
            >
    ...
    <img
        ref="printableImage"
        class="printable-area screen-hidden"
        alt="Printable Layout"
    />
</template>
        ...
        reader.onload = (e) => {
            ...
            img.onload = () => {
                ...
                if (width > MAX_DIMENSION || height > MAX_DIMENSION) {
                    ...
                    console.log(
                        `Image downscaled from ${img.width}x${img.height} to ${width}x${height}`,
                    );
                } else {
            ...
            img.onerror = () => {
                console.error(
                    "SingleBadgeLayout: Image failed to load from FileReader result.",
                    { src: e.target.result },
                );
...
const generateLayout = () => {
    ...
    if (layoutMode.value === "auto") {
        ...
        const marginTopPx = mmToPx(marginTopMM),
            marginLeftPx = mmToPx(marginLeftMM),
            marginRightPx = mmToPx(marginRightMM),
            marginBottomPx = mmToPx(marginBottomMM);
        const gapHPx = mmToPx(gapH.value),
            gapVPx = mmToPx(gapV.value);
    ...
    img.onload = () => {
        let drawnCount = 0;
        const maxToDraw =
            printQuantity.value && printQuantity.value > 0
                ? printQuantity.value
                : rows * cols;
        ...
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                ...
                ctx.drawImage(
                    img,
                    x + (outerDiaPx - innerDiaPx) / 2,
                    y + (outerDiaPx - innerDiaPx) / 2,
                    innerDiaPx,
                    innerDiaPx,
                );
                ctx.restore();
